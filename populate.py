...
def highest_rank_calculator():

    OUTPUT_DIRECTORY = '/Users/EdmundLudlow/Desktop/Programming/actor_dashboard/bfi_spreadsheets/spreadsheet_directory_2/'
    LIST_OF_DATAFRAMES = []
    
    for xls_file_2020 in os.listdir(OUTPUT_DIRECTORY):      
        print(xls_file_2020)
        if str(xls_file_2020) == '.DS_Store':
            continue
        else:
            dfs_2020 = pd.read_excel(OUTPUT_DIRECTORY + xls_file_2020, header = None)

        #Format dataframes
        #Correct column index line
        if 'Rank' in dfs_2020.columns:
                continue
        else:
            #Find the row with the desired header
            index_row = dfs_2020.index[dfs_2020.iloc[:,0] == 'Rank'].tolist()
            #Move Header row to index
            dfs_2020 = set_column_index(dfs_2020, index_row[0])
        
        dfs_2020.dropna(subset = ['Rank', 'Total Gross to date'], inplace=True)

        for index, row in dfs_2020.iterrows():
            film_from_df = row['Film']
            rank_from_df = row['Rank']
        
            try:
                film_instance = Film.objects.get(title = str(film_from_df.upper()))
                print(film_instance.title)
            except:
                continue

            print(film_instance.best_rank)
            print(rank_from_df)

            if film_instance.best_rank == None:
                film_instance.best_rank = int(rank_from_df)

            elif int(film_instance.best_rank) == 0:
                film_instance.best_rank = int(rank_from_df)

            elif int(film_instance.best_rank) > int(rank_from_df):
                film_instance.best_rank = int(rank_from_df)
...
def populate_november_december():

    SSID = '6cf99b5e'

    OUTPUT_DIRECTORY = '/Users/EdmundLudlow/Desktop/Programming/actor_dashboard/bfi_spreadsheets/spreadsheet_directory_3/'  
    LIST_OF_DATAFRAMES = []
            ...
            if existing_film:
                ...
                if not data:
                    ...
                    if int(data['Year']) < 2001:
                        ...
                        xls_file_2020_split = xls_file_2020.split("-")
                        xls_file_date = xls_file_2020_split[5] + "-" + xls_file_2020_split[6] + "-" + xls_file_2020_split[7]
                        xls_date_corrected = datetime.strptime(xls_file_date, '%Y-%m-%d').date() 
